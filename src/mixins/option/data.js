...
export default {
	...
	methods: {
		getShowChipList(items, range = 100) {
			...
			const startIndex = neerIndex - range < 0 ? 0 : neerIndex - range
	...
	computed: {
		...
		centerPoint() {
			...
			let mustNeer = 0
			this.allItems.forEach((item) => {
				mustNeer = Math.abs(refPoint - item) < Math.abs(refPoint - mustNeer) ? item : mustNeer
			})
			return mustNeer
		},
		showChipList() {
			return this.centerPoint && this.range ? this.getShowChipList(this.allItems, this.range) : null
		},
					...
					(result, item) => {
						if (!this.showChipList || this.showChipList.indexOf(item) > -1) {
							result.push({
								item,
								C:
									_(this.groupItemTypeItemInformed).getVal(`${item}.C.chip_valume`, 0) +
									_(this.groupItemTypeAccumulation).getVal(`${item}.C.total_chip`, 0),
								P:
									_(this.groupItemTypeItemInformed).getVal(`${item}.P.chip_valume`, 0) +
