...
class DateService {
  ...
  static async getWednesdayContinueHoliday() {
    return _.map(
      await DB.table("wednesday_continue_holiday").select("date"),
      "date"
    );
  }
  ...
  static async getSpecialMainWeek() {
    return (await DB.table("sys")
      .select("special_main_week")
      .first()).special_main_week;
  }
  ...
  async getDateInfo({ request }) {
    ...
    while (
      DateService.inTheseDay(
        dateTime,
        await DateService.getWednesdayContinueHoliday()
      )
    ) {
    ...
    const is_month_item_end_week = (await DB.table("sys")
      .select("is_month_item_end_week")
      .first()).is_month_item_end_week;
    if (is_month_item_end_week != 0) {
      ...
      switch (is_month_item_end_week) {
        ...
        case 3:
          ...
          break;
        case 4:
          startMethod = this.getFourthWednesday;
          endMethod = this.getFifthWednesday;
          break;
      }
      ...
      Math.ceil(this.getThirdWednesday(moment(dateTime)).date() / 7) > 2
    ) {
      monthSettleStartTime = this.getSecondWednesday(
        moment(dateTime)
      ).getNormalStartTime(); //.format('YYYY-MM-DD 15:00:00')
      monthSettleEndTime = this.getThirdWednesday(
        moment(dateTime)
      ).getNormalEndTime(); //.format('YYYY-MM-DD 13:45:00')
    } else {
      monthSettleStartTime = this.getThirdWednesday(
        moment(dateTime)
      ).getNormalStartTime(); //.format('YYYY-MM-DD 15:00:00')
      monthSettleEndTime = this.getFourthWednesday(
        moment(dateTime)
      ).getNormalEndTime(); //.format('YYYY-MM-DD 13:45:00')
    }
    ...
    // } else {
    weekSettleStartTime = moment(dateTime)
      .day(3)
      .getMorningStartTime(); //.format('YYYY-MM-DD 08:45:00')
    weekSettleEndTime = moment(dateTime)
      .day(3)
      .getNormalEndTime(); //.format('YYYY-MM-DD 13:45:00')
    // }

    const isMonthSettleTime = moment(dateTime).isBetween(
  ...
  // 五週三
  getFifthWednesday(targetMoment) {
    return moment(targetMoment.format("YYYY-MM-01")).day(31);
  }
      ...
      case "1":
        return (
          now.isBetween(
            ...
            now.format("YYYY-MM-DD 13:45:10")
          ) ||
          now.isBetween(
      ...
      case "5":
        return (
          now.isBetween(
            ...
            now.format("YYYY-MM-DD 05:00:10")
          ) ||
          now.isBetween(
            now.format("YYYY-MM-DD 08:45:00"),
            now.format("YYYY-MM-DD 13:45:10")
